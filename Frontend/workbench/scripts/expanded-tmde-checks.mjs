...
export async function checkExpandedTmde({ frame, page, saved }) {
  ...
  for (const kind of ['Table', 'Equation']) {
    ...
    const sourceStyle = await remove.evaluate(el => ({ color: getComputedStyle(el).color, background: getComputedStyle(el).backgroundColor }));
    await table.locator("tr[data-range-id]:not([data-uncertainty-source-id]) .range-row-cell").first().hover();
    await rangeRemove.hover();
    await setTimeout(150);
    assert.deepEqual(await rangeRemove.evaluate(el => ({ color: getComputedStyle(el).color, background: getComputedStyle(el).backgroundColor })), sourceStyle, 'source delete uses the same red hover as range delete');
    await row.locator('.range-row-cell').hover();
    await click(remove);
